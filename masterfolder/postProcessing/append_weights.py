"""
append_weights.py -- attach stroopwafel mixture weights to the COMPAS output.

Stroopwafel does not sample from the birth distribution, so every system carries
a `mixture_weight` that undoes the sampling bias. This script sorts the merged
COMPAS h5 by SEED, matches it against stroopwafel's samples.csv, and writes a new
file with a `mixture_weight` column added to BSE_System_Parameters and
BSE_Double_Compact_Objects.

ONLY meaningful for stroopwafel runs. A plain grid run samples from the birth
distribution already, so every system has weight 1 and you skip this step.

    python3 append_weights.py --data-dir ../MainRun/

See ../../docs/04_postprocessing.md
"""
import argparse
import numpy as np
import h5py as h5
...
Dir      = '../MainRun/' # ! specific to structure of your directories
SW_name  = 'samples.csv'
...
###
##################################################################
def main(data_dir = None, Stroopwafel_name = None, Raw_COMPAS_name = None, Weights_COMPAS_name = None):
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Append stroopwafel mixture weights to a merged COMPAS h5 file.")
    parser.add_argument('--data-dir', default=Dir,
                        help='directory holding both the COMPAS h5 and samples.csv '
                             f'(default {Dir})')
    parser.add_argument('--sw-name', default=SW_name,
                        help=f'stroopwafel sample file (default {SW_name})')
    parser.add_argument('--raw-name', default=Raw_name,
                        help=f'merged COMPAS output to read (default {Raw_name})')
    parser.add_argument('--new-name', default=New_name,
                        help=f'weighted output to write (default {New_name})')
    args = parser.parse_args()

    data_dir = args.data_dir if args.data_dir.endswith('/') else args.data_dir + '/'
    main(data_dir=data_dir, Stroopwafel_name=args.sw_name,
         Raw_COMPAS_name=args.raw_name, Weights_COMPAS_name=args.new_name)
